...
def tagtext(oNode=None, cTag=None):
    try:
        xpath = ".//{http://www.portalfiscal.inf.br/mdfe}%s" % (cTag)
...
class mdfe(object):
    def __init__(
        ...
        self.canvas.setTitle("MDFE")
        self.canvas.setStrokeColor(black)

        for oXML in list_xml:
            oXML_cobr = oXML.find(".//{http://www.portalfiscal.inf.br/mdfe}cobr")

            self.NrPages = 1
            self.Page = 1

            el_det = oXML.findall(".//{http://www.portalfiscal.inf.br/mdfe}det")
            ...
            if el_det is not None:
                ...
                for nId, item in enumerate(el_det):
                    el_prod = item.find(".//{http://www.portalfiscal.inf.br/mdfe}prod")
                    infAdProd = item.find(
                        ".//{http://www.portalfiscal.inf.br/mdfe}infAdProd"
                    )
    ...
    def ide_emit(self, oXML=None, timezone=None):
        elem_infMDFe = oXML.find(".//{http://www.portalfiscal.inf.br/mdfe}infMDFe")
        elem_protMDFe = oXML.find(".//{http://www.portalfiscal.inf.br/mdfe}protMDFe")
        elem_emit = oXML.find(".//{http://www.portalfiscal.inf.br/mdfe}emit")
        elem_ide = oXML.find(".//{http://www.portalfiscal.inf.br/mdfe}ide")
        elem_evento = oXML.find(".//{http://www.portalfiscal.inf.br/mdfe}infEvento")

        cChave = elem_infMDFe.attrib.get("Id")[3:]
        ...
        self.stringcenter(
            self.nLeft + 152,
            self.nlin + 28,
            "www.mdfe.fazenda.gov.br/portal ou no site da SEFAZ Autorizadora",
        )
        ...
        cDt, cHr = getdateByTimezone(
            tagtext(oNode=elem_protMDFe, cTag="dhRecbto"), timezone
        )
        cProtocolo = tagtext(oNode=elem_protMDFe, cTag="nProt")
    ...
    def destinatario(self, oXML=None, timezone=None):
        elem_ide = oXML.find(".//{http://www.portalfiscal.inf.br/mdfe}ide")
        elem_dest = oXML.find(".//{http://www.portalfiscal.inf.br/mdfe}dest")
    ...
    def entrega_retirada(self, oXML=None, timezone=None):
        elem_entrega = oXML.find(".//{http://www.portalfiscal.inf.br/mdfe}entrega")
        elem_retirada = oXML.find(".//{http://www.portalfiscal.inf.br/mdfe}retirada")
        self.canvas.setFont("NimbusSanL-Bold", 7)
    ...
    def faturas(self, oXML=None, timezone=None):
        ...
        if elem_infAdic is not None:
            codvend = elem_infAdic.find(
                ".//{http://www.portalfiscal.inf.br/mdfe}obsCont\
[@xCampo='CodVendedor']"
            )
            self.string(
...
obsCont[@xCampo='NomeVendedor']"
    ...
    def impostos(self, oXML=None):
        # Impostos
        el_total = oXML.find(".//{http://www.portalfiscal.inf.br/mdfe}total")
        nMr = self.width - self.nRight
        self.nlin += 1
        self.canvas.setFont("NimbusSanL-Bold", 7)
    ...
    def transportes(self, oXML=None):
        el_transp = oXML.find(".//{http://www.portalfiscal.inf.br/mdfe}transp")
        veic_transp = oXML.find(".//{http://www.portalfiscal.inf.br/mdfe}veicTransp")
        nMr = self.width - self.nRight
    ...
    def produtos(
        ...
        for id in range(index, max_index + 1):
            ...
            item = el_det[id]
            el_prod = item.find(".//{http://www.portalfiscal.inf.br/mdfe}prod")
            el_imp = item.find(".//{http://www.portalfiscal.inf.br/mdfe}imposto")

            el_imp_ICMS = el_imp.find(".//{http://www.portalfiscal.inf.br/mdfe}ICMS")
            el_imp_IPI = el_imp.find(".//{http://www.portalfiscal.inf.br/mdfe}IPI")
    ...
    def calculo_issqn(self, oXML=None):
        elem_emit = oXML.find(".//{http://www.portalfiscal.inf.br/mdfe}emit")
        el_total = oXML.find(".//{http://www.portalfiscal.inf.br/mdfe}total")
        issqn_total = el_total.find(".//{http://www.portalfiscal.inf.br/mdfe}ISSQNtot")
        if not issqn_total:
            return 0
    ...
    def adicionais(self, oXML=None, tamanho_diminuir=0):
        el_infAdic = oXML.find(".//{http://www.portalfiscal.inf.br/mdfe}infAdic")
    ...
    def recibo_entrega(self, oXML=None, timezone=None):
        el_ide = oXML.find(".//{http://www.portalfiscal.inf.br/mdfe}ide")
        el_dest = oXML.find(".//{http://www.portalfiscal.inf.br/mdfe}dest")
        el_total = oXML.find(".//{http://www.portalfiscal.inf.br/mdfe}total")
        el_emit = oXML.find(".//{http://www.portalfiscal.inf.br/mdfe}emit")
    ...
    def _generate_cce(self, cce_xml=None, oXML=None, timezone=None):
        ...
        # values
        infMDFe = oXML.find(".//{http://www.portalfiscal.inf.br/mdfe}infMDFe")
        res_partner = infMDFe.find(".//{http://www.portalfiscal.inf.br/mdfe}xNome")

        elem_infMDFe = cce_xml.find(".//{http://www.portalfiscal.inf.br/mdfe}infEvento")

        res_partner = tagtext(oNode=infMDFe, cTag="xNome")
        self.string(82, 18, res_partner)
        cnpj = format_cnpj_cpf(tagtext(oNode=elem_infMDFe, cTag="CNPJ"))
        self.string(82, 24, cnpj)
        chave_acesso = tagtext(oNode=elem_infMDFe, cTag="chMDFe")
        self.string(82, 30, chave_acesso)
        data_correcao = getdateByTimezone(
            tagtext(oNode=elem_infMDFe, cTag="dhEvento"), timezone
        )
        data_correcao = data_correcao[0] + "  " + data_correcao[1]
        self.string(82, 36, data_correcao)
        cce_id = elem_infMDFe.values()[0]
        self.string(82, 42, cce_id)

        correcao = tagtext(oNode=elem_infMDFe, cTag="xCorrecao")
        ...
        condicoes = tagtext(oNode=elem_infMDFe, cTag="xCondUso")
